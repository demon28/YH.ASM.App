<template>
	...
	<view class="content">
		...
		<view class="uni-list" style="margin-top: 20upx;">
			<label class="uni-list-cell uni-list-cell-pd" style=" min-height: 60upx; height: 60upx;" v-for="item in list" :key="item.sid">
				<view>
					<view> {{item.content}}</view>
				</view>
				<view>
...
<script>
	...
	export default {
		...
		computed: mapState(['reportSupport']),
		components: {},
		data() {
			return {
				list: [],
		...
		methods: {
			...
			Init() {
				...
				let model={
				 ...
				 let path="/api/Support/ListSupport";
				...
				uni.request({
					...
					success: (res) => {
						...
						for (var i = 0; i < res.data.Content.length; i++) {
							...
							let content = res.data.Content[i].CONTENT;
							if(content.length>15){
								content=content.substring(0,15)+"...";
							}		
									
							let sid=res.data.Content[i].SID;
